...
- `b8ba637c` `feat(foundation): store startup context in project config` - completed the project-owned startup-context move by storing tenant metadata in per-project `.power-desk.config.json` files, updating startup/session flows to read selected-project context, and documenting the shift in `docs/adr-0005-project-startup-context-config.md`.
- `82678974` `fix(web): open startup config in available editor` - hardened the project startup-config open actions so Settings and sidebar flows resolve an installed editor instead of failing on stale terminal-editor preferences.
- `30f51ffe` `feat(foundation): move app settings into server state` - completed the user-scoped non-secret configuration model by moving app settings into server-owned `settings.json`, exposing typed get/update APIs, and documenting the boundary in `docs/adr-0006-server-owned-user-settings.md`.
