import {
  ...
  type ServerSessionStateUpdatedPayload,
  WS_CHANNELS,
  WS_METHODS,
...
const serverSessionStateUpdatedListeners = new Set<
  (payload: ServerSessionStateUpdatedPayload) => void
>();
...
export function onServerSessionStateUpdated(
  listener: (payload: ServerSessionStateUpdatedPayload) => void,
): () => void {
  serverSessionStateUpdatedListeners.add(listener);

  const latestSessionState =
    instance?.transport.getLatestPush(WS_CHANNELS.serverSessionStateUpdated)?.data ?? null;
  ...
  return () => {
    serverSessionStateUpdatedListeners.delete(listener);
  };
}
...
export function createWsNativeApi(): NativeApi {
  ...
  transport.subscribe(WS_CHANNELS.serverSessionStateUpdated, (message) => {
    const payload = message.data;
    for (const listener of serverSessionStateUpdatedListeners) {
      try {
        listener(payload);
      } catch {
        // Swallow listener errors
